...
; Johto Gym Leader TM gifts
	...
	const EVENT_GOT_TM49_BUG_BITE
	const EVENT_GOT_TM01_DRAIN_PUNCH
	const EVENT_GOT_TM45_ATTRACT
	const EVENT_GOT_TM30_SHADOW_BALL
	const EVENT_GOT_TM23_IRON_HEAD
	const EVENT_GOT_TM16_ICICLE_CRASH
	const EVENT_GOT_TM24_DRAGON_CLAW
; HMs (EVENT_GOT_HM07_WATERFALL is with the Johto itemballs)
	...
	const EVENT_ILEX_FOREST_TART_APPLE
; Rods
...
; Johto story events
	...
	const EVENT_ILEX_FOREST_SWEET_APPLE
	const EVENT_ILEX_FOREST_SYRUPY_APPLE
	const EVENT_HEALED_MOOMOO
	const EVENT_GOT_TM13_WORK_UP_FROM_MOOMOO_FARM
	...
	const EVENT_GOT_TM50_HONE_CLAWS
	...
	const EVENT_GOT_TWISTEDSPOON_FROM_SANTOS
...
; Johto hidden items
	...
	const EVENT_SHIVER_ISLE_ICE_STONE
	const EVENT_BEAT_BOARDER_AIDAN
	const EVENT_BEAT_BOARDER_NOEL
...
; Crystal-exclusive events in Johto
	...
	const EVENT_BEAT_SKIER_BIANCA
	const EVENT_BEAT_COOLTRAINERM_LARRY
	const EVENT_BEAT_COOLTRAINERM_SNOW
	const EVENT_BEAT_COOLTRAINERM_CYANIDE
	const EVENT_ILEX_FOREST_HIDDEN_RELIC_CLOCK
	const EVENT_GOT_RIOLU_FROM_GOLDENROD_UNDERGROUND_GIRL
; Kanto story events
...
; ???
	const EVENT_BEAT_FALKNER_REMATCH
	const EVENT_BEAT_BUGSY_REMATCH
	const EVENT_BEAT_WHITNEY_REMATCH
	const EVENT_BEAT_MORTY_REMATCH
	const EVENT_BEAT_CHUCK_REMATCH
	const EVENT_BEAT_JASMINE_REMATCH
...
; Phone events
	...
	const EVENT_286
	const EVENT_MAHOGANY_TOWN_LANCE_AND_DRAGONITE
	const EVENT_288
...
; More Johto story events
	...
	const EVENT_CONTEST_OFFICER_HAS_STONE
	const EVENT_CONTEST_OFFICER_HAS_PP_UP
	const EVENT_CONTEST_OFFICER_HAS_APRICORN
...
; More Crystal-exclusive events in Johto
	...
	const EVENT_BEAT_EUSINE_IN_ECRUTEAK_CITY
	const EVENT_GOT_GS_BALL_FROM_POKECOM_CENTER
; ???
	const EVENT_ECRUTEAK_CITY_EUSINE
	const EVENT_342
	const EVENT_343
	const EVENT_344
	const EVENT_345
...
; Boarder
	...
	const EVENT_BEAT_BOARDER_SPENCER
; Sage
...
; Burglar
	...
	const EVENT_BEAT_BURGLAR_MORTON
	const EVENT_BEAT_BURGLAR_VANCE
; Unknown
	const EVENT_430
; Biker
...
; Firebreather
	...
	const EVENT_BEAT_FIREBREATHER_SCORCH
	const EVENT_BEAT_FIREBREATHER_TORCH
; Fisher
...
; Picnicker
	...
	const EVENT_BEAT_SCHOOL_GIRL_KIM
	const EVENT_BEAT_PICNICKER_CINDY
	const EVENT_BEAT_SCHOOL_GIRL_HOPE
	const EVENT_BEAT_SCHOOL_GIRL_SHARON
	const EVENT_BEAT_SCHOOL_GIRL_DEBRA
	...
	const EVENT_BEAT_SCHOOL_GIRL_HEIDI
	const EVENT_BEAT_SCHOOL_GIRL_EDNA
	...
	const EVENT_BEAT_SCHOOL_GIRL_TANYA
...
; Lass
	...
	const EVENT_BEAT_LASS_DANIELLE
	const EVENT_BEAT_LASS_KATHRYN
; Hiker
...
; ???
	const EVENT_BEAT_KIMONO_GIRL_FUYU
	const EVENT_BEAT_KIMONO_GIRL_HANA
	const EVENT_BEAT_KIMONO_GIRL_YUMI
	const EVENT_5CF
	const EVENT_5D0
	const EVENT_5D1
	const EVENT_5D2
	...
	const EVENT_5FE
	const EVENT_5FF
	const EVENT_BEAT_CRYSTAL_VIOLET_CITY
	const EVENT_CRYSTAL_VIOLET_INITIALIZED
	const EVENT_BEAT_CRYSTAL_ILEX_FOREST
	const EVENT_CRYSTAL_ILEX_INITIALIZED
	const EVENT_ILEX_FOREST_CRYSTAL
	const EVENT_ILEX_FOREST_HEARD_TICKING
	const EVENT_ILEX_FOREST_TICKING_FADED
	const EVENT_607
	const EVENT_BEAT_CRYSTAL_CIANWOOD_CITY
	const EVENT_CRYSTAL_CIANWOOD_INITIALIZED
	const EVENT_CIANWOOD_CITY_CRYSTAL
	const EVENT_BEAT_CRYSTAL_ICE_PATH
	const EVENT_CRYSTAL_ICE_PATH_INITIALIZED
	const EVENT_ICE_PATH_CRYSTAL
	const EVENT_BEAT_CRYSTAL_CERULEAN_CAPE
	const EVENT_GOT_CRYSTALS_NUMBER ; reused unused slot (was EVENT_CRYSTAL_TOHJO_FALLS_INITIALIZED)
	const EVENT_ROUTE_25_CRYSTAL
	const EVENT_611
	const EVENT_612
	const EVENT_613
	const EVENT_614
...
; Johto itemballs
	...
	const EVENT_LAKE_OF_RAGE_TM_DRAGON_DANCE
	...
	const EVENT_OLIVINE_LIGHTHOUSE_5F_TM_FLASH_CANNON
	...
	const EVENT_GOLDENROD_DEPT_STORE_B1F_BRICK_PIECE
	const EVENT_GOLDENROD_UNDERGROUND_WAREHOUSE_MAX_ETHER
	const EVENT_GOLDENROD_UNDERGROUND_WAREHOUSE_TM_KNOCK_OFF
	...
	const EVENT_MOUNT_MORTAR_2F_INSIDE_TM_BULK_UP
	...
	const EVENT_DARK_CAVE_BLACKTHORN_ENTRANCE_TM_NIGHT_SLASH
	...
	const EVENT_ROUTE_35_TM_ROCK_TOMB
...
; Johto people
	...
	const EVENT_OLIVINE_GYM_LASS_DANIELLE
	const EVENT_OLIVINE_GYM_LASS_KATHRYN
	...
	const EVENT_VIOLET_CITY_CRYSTAL
...
; Kanto itemballs
	...
	const EVENT_ROCK_TUNNEL_1F_TM_POWER_GEM
...
; New to Crystal
	...
	const EVENT_BEAT_HEX_MANIAC_WINNIE
	const EVENT_BEAT_JUGGLER_MARCO
	const EVENT_BEAT_JUGGLER_LEON
	const EVENT_BEAT_COSPLAYER_DAISY
	const EVENT_BEAT_COSPLAYER_MIMI
	const EVENT_BEAT_COSPLAYER_PEARL
	const EVENT_BEAT_COSPLAYER_PIXIE
	const EVENT_BEAT_AGATHA
	const EVENT_BEAT_LORELEI
	const EVENT_BEAT_RED2
	const EVENT_BEAT_BLUE_CLOAK
	const EVENT_BEAT_GREEN
	const EVENT_BLUE_CLOAK_IN_CINNABAR
	const EVENT_GREEN_IN_ROUTE20
	...
	const EVENT_PAID_SAFARI_ZONE_FEE
	const EVENT_BEAT_COSPLAYER_NOELLE
	const EVENT_BEAT_JUGGLER_SILAS
	const EVENT_BEAT_SCHOOL_GIRL_NADIA
	const EVENT_BEAT_TAMER_COLE
	const EVENT_BEAT_TAMER_JAX
	const EVENT_BEAT_TAMER_RIGBY
	const EVENT_BEAT_BATTLE_GIRL_MIHO
	const EVENT_BEAT_BATTLE_GIRL_AYA
	const EVENT_BEAT_LASS_PAIGE
	const EVENT_BEAT_POKEFANM_DUSTIN
	const EVENT_BEAT_HEX_MANIAC_ODESSA
	const EVENT_BEAT_HEX_MANIAC_LILITH
	const EVENT_MET_GRAVEKEEPER
	const EVENT_MEDIUM_GAVE_ADVICE
	const EVENT_CLEANED_GRAVE_1
	const EVENT_CLEANED_GRAVE_2
	const EVENT_GOT_SPELL_TAG_FROM_GRAVEKEEPER
	const EVENT_GOT_METAL_COAT_FROM_JASMINE
	const EVENT_TOHJO_FALLS_SUICUNE
	const EVENT_FOUGHT_TOHJO_FALLS_SUICUNE
	const EVENT_LAKE_OF_RAGE_RAIKOU
	const EVENT_FOUGHT_LAKE_OF_RAGE_RAIKOU
	const EVENT_SILVER_CAVE_ROOM_2_MEWTWO
	const EVENT_BEAT_RIVAL_AT_INDIGO_PLATEAU
	const EVENT_ROUTE_25_MEW
	const EVENT_ROUTE_25_MEW_APPEARED
	const EVENT_CRYSTAL_CAUGHT_MEW ; legacy: cleared on Route 25 for save compatibility
	const EVENT_ROUTE_25_CAUGHT_MEW
	const EVENT_ROUTE_25_CRYSTAL_LEFT

; Route 41 FINIZEN (Sundays only; see maps/Route41.asm)
	const EVENT_ROUTE_41_FINIZEN
...
; Route 19 elemental bird islands
	const EVENT_ROUTE_19_ELEMENTAL_SPHERE_PLACED
...
; Victory Road trainers
	const EVENT_BEAT_TAMER_DEVIN
	const EVENT_BEAT_TAMER_ROLF
	const EVENT_BEAT_BATTLE_GIRL_KIRA
	const EVENT_BEAT_BATTLE_GIRL_MINA
...
; Fossils hidden in the Ruins of Alph Aerodactyl word room.
	const EVENT_AERODACTYL_WORD_ROOM_CLAW_WALL_OPEN
	const EVENT_AERODACTYL_WORD_ROOM_ARMOR_WALL_OPEN
	const EVENT_AERODACTYL_WORD_ROOM_SAIL_WALL_OPEN
	const EVENT_PICKED_UP_CLAW_FOSSIL_FROM_AERODACTYL_WORD_ROOM
	const EVENT_PICKED_UP_ARMOR_FOSSIL_FROM_AERODACTYL_WORD_ROOM
	const EVENT_PICKED_UP_SAIL_FOSSIL_FROM_AERODACTYL_WORD_ROOM

; Fossils hidden in the Ruins of Alph Ho-Oh word room.
	const EVENT_HO_OH_WORD_ROOM_SKULL_WALL_OPEN
	const EVENT_HO_OH_WORD_ROOM_COVER_WALL_OPEN
	const EVENT_HO_OH_WORD_ROOM_JAW_WALL_OPEN
	const EVENT_PICKED_UP_SKULL_FOSSIL_FROM_HO_OH_WORD_ROOM
	const EVENT_PICKED_UP_COVER_FOSSIL_FROM_HO_OH_WORD_ROOM
	const EVENT_PICKED_UP_JAW_FOSSIL_FROM_HO_OH_WORD_ROOM

; Fossils hidden in the Ruins of Alph Kabuto word room.
	const EVENT_KABUTO_WORD_ROOM_DOME_WALL_OPEN
	const EVENT_KABUTO_WORD_ROOM_HELIX_WALL_OPEN
	const EVENT_KABUTO_WORD_ROOM_ROOT_WALL_OPEN
	const EVENT_PICKED_UP_DOME_FOSSIL_FROM_KABUTO_WORD_ROOM
	const EVENT_PICKED_UP_HELIX_FOSSIL_FROM_KABUTO_WORD_ROOM
	const EVENT_PICKED_UP_ROOT_FOSSIL_FROM_KABUTO_WORD_ROOM

; Fossils hidden in the Ruins of Alph Omanyte word room.
	const EVENT_OMANYTE_WORD_ROOM_AMBER_WALL_OPEN
	const EVENT_OMANYTE_WORD_ROOM_PLUME_WALL_OPEN
	const EVENT_PICKED_UP_OLD_AMBER_FROM_OMANYTE_WORD_ROOM
	const EVENT_PICKED_UP_PLUME_FOSSIL_FROM_OMANYTE_WORD_ROOM
