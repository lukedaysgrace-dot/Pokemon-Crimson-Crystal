...
## 1b. Cross-bank plain `call`s — the freeze (fixed)

Found after the first pass, from a live report: *"used Sucker Punch, it said blew
away spikes, then froze."*

`farcall` switches ROM banks; a plain `call` does not. When the move expansion
relocated code into new sections, eleven call sites kept plain `call`/`jp` to
symbols that the linker placed in a **different bank** — so at runtime they
jumped to that address with the wrong bank paged in and executed whatever data
happened to sit there. This is invisible to both the assembler and the linker.

The fatal one:

```
PlayerAttackDamage / EnemyAttackDamage   (bank 13)
	call GetPhysicalAttackSource         -> bank 122
```

`GetPhysicalAttackSource` lives in `handoff_moves.asm`. It runs on **every
physical attack in the game**, so any physical move — Sucker Punch included —
jumped into garbage. Four sites.

The other seven were all in `handoff_moves.asm` (bank 122):

| Site | Target | Bank |
|---|---|---|
| `BattleCommand_StealthRock` | `AnimateCurrentMove`, `FailMove` | 13 |
| `BattleCommand_StickyWeb` | `AnimateCurrentMove`, `FailMove` | 13 |
| `BattleCommand_Defog` | `AnimateCurrentMove` | 13 |
| `StealthRockDamage_Core` | `GetEighthMaxHP`, `SubtractHPFromTarget` | 15 |

So Stealth Rock, Sticky Web and Defog each froze on use, and Stealth Rock's
switch-in damage froze on any switch once it was up.

**These are pre-existing** — the same bank split is present in a build of
`acad5baa`, before any of my changes. They were not caused by the audit pass.

**Fixed by:**

* `GetPhysicalAttackSource` / `...Boosted` moved into `effect_commands.asm` so
  they share a bank with their callers. They take *and return* `hl`, and the
  `farcall` macro clobbers `hl` on entry, so co-location is the only correct fix
  here — not a `farcall` conversion.
* To make room, `BattleCommand_TrapTarget`'s body was relocated to
  `effect_commands_core.asm` as `BattleTrapTarget_Core` — finishing the move the
  interrupted commit had started. It only calls home-bank routines, so it
  relocates cleanly. Effect Commands went from 9 to 26 bytes free.
* The seven `handoff_moves.asm` sites converted to `farcall`, with `jp FailMove`
  becoming `farcall FailMove` + `ret`.

Verified afterwards: a bank-aware scan of every `call`/`jp` in `engine/`
(resolving each site's bank from the linker symbol table) reports **zero**
remaining cross-bank plain calls.

This class of bug is worth re-checking after any change that moves code between
sections, because the linker packs floating sections by size — 29 sections
changed bank between the two builds I compared.
