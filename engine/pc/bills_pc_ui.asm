...
.Text_GottaHavePokemon:
	...
	text_far BillsPCText_GottaHavePokemon
	text_end
BillsPC_LoadUI:
	ld a, 1
	ldh [rVBK], a
	...
	ld de, BillsPC_ObjGFX
	ld hl, vTiles3 tile $24
	...
	ld de, BillsPC_GenderShinyGFX
	ld hl, vTiles2 tile $42
	lb bc, BANK(BillsPC_GenderShinyGFX), 3
	call Get2bpp
...
.not_holding
	...
	jr .got_hi
.egg
	ld a, LOW(PC_EGG_INDEX)
	ld [wCurIcon], a
	ld [hli], a
	ld a, HIGH(PC_EGG_INDEX)
.got_hi
	ld [wCurIconForm], a
	ld [hli], a
	ld a, e
...
.delay_loop
	; Delay first before loading the frontpic. Retry if it puts us too late.
	call DelayFrame
	ldh a, [rLY]
	cp $13
	jr nc, .delay_loop
...
; Updates storage and potentially switches Mewtwo form if item changed.
	...
	ld [wCurPartySpecies], a
	ld de, wTempMonItem
	ld hl, wTempMonIndex + 1
	ld a, [hl]
	ld b, a
	push bc
	call BillsPC_DoNothing
...
.ReleasedXMon:
	text "Released "
	text_decimal wTempByteValue, 1, 2
	line "#mon."
	prompt
...
BillsPC_GameSaved:
	text_far BillsPCText_SavedTheGame
	text_end
...
BillsPC_LCDCode:
; (assembled in ROM, copied to wBillsPC_LCDCodeBuffer at runtime)
BillsPC_LCDCode_1:
	; Write boxmon palettes
	ldh a, [rSTAT]
	bit 2, a ; STAT bit 2: LYC coincidence
	jr z, .donepc
	push hl
	push bc
	ld c, LOW(rBGPD)
...
endr
	...
	ld a, LOW(wBillsPC_LCDCodeBuffer + (BillsPC_LCDCode_2 - BillsPC_LCDCode))
	ldh [hLCDInterruptFunctionTargetLo], a
	ld a, HIGH(wBillsPC_LCDCodeBuffer + (BillsPC_LCDCode_2 - BillsPC_LCDCode))
	ldh [hLCDInterruptFunctionTargetHi], a
	pop bc
	pop hl
.donepc
